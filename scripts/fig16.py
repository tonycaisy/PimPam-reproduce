...
csv_path = './results/fig16.csv'
with open('./results/pangolin_time.txt', 'r') as f:
    pangolin_time = f.readlines()
    pangolin_time = [float(x.strip()) for x in pangolin_time]
    pangolin_time = [math.log10(x*1e6) for x in pangolin_time]
pangolin_time.append(0.0) # LJ for C5 is out of memory

pimpam_time = []
with open('./results/pimpam_time.txt', 'r') as f1:
    times = f1.readlines()[::5]
    times = [float(x.strip()) for x in times]
    times = [math.log10(x*1e6) for x in times]
    with open('./results/pimpam_bitmap.txt', 'r') as f2:
        for time in times:
            pimpam_time.append(time)
            time1 = f2.readline()
            time2 = f2.readline()
            time1 = float(time1.strip())
            time2 = float(time2.strip())
            time1 = math.log10(time1*1e6)
            time2 = math.log10(time2*1e6)
            pimpam_time.append(time1)
            pimpam_time.append(time2)

dataset_list = ['WV', 'PP', 'AP', 'YT', 'PT', 'LJ']
with open(csv_path, 'w') as f:
    f.write('Name,GPU,MyMiner\n')
    for graph in dataset_list:
        for pattern in ['CLIQUE3', 'CLIQUE4', 'CLIQUE5']:
            f.write(graph + '-' + pattern + ',')
            f.write(','.join([str(x) for x in [pangolin_time.pop(0), pimpam_time.pop(0)]]) + '\n')
...
fig_name = 'fig16' # TODO: figure name, used as pdf name


workload_list = csv_hd["Name"].to_list()
scheme_list = ['GPU', 'MyMiner'] # TODO: list of all schemes in the breakdown. Each column corresponds with one scheme
...
fig_ylim = (0, 10) # TODO: y-axis range. Align with your data
...
for i in range(0,6):
    next_ticks.append((fig_xticks[i*3] + fig_xticks[(i+1)*3-1])/2+0.02)
print(next_ticks)
...
line_ticks = []
for i in range(0,5):
    line_ticks.append((fig_xticks[(i+1)*3-1] + fig_xticks[(i+1)*3])/2+0.02)
