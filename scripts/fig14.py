...
csv_path = './results/fig14.csv'
dataset_list = ['WV', 'PP', 'AP', 'YT', 'PT', 'LJ']
with open(csv_path, 'w') as f:
    with open('./results/pimpam_no_bitmap.txt', 'r') as f1:
        with open('./results/pimpam_bitmap.txt', 'r') as f2:
            f.write("Name,W/o Dynamic Bitmaps,MyMiner\n")
            for graph in dataset_list:
                for pattern in ['CLIQUE4', 'CLIQUE5']:
                    f.write(graph + '-' + pattern + ',')
                    line1 = float(f1.readline().strip())
                    line2 = float(f2.readline().strip())
                    f.write('1,'+str(line1/line2)+'\n')
...
fig_name = 'fig14' # TODO: figure name, used as pdf name


workload_list = csv_hd["Name"].to_list()
scheme_list = ['W/o Dynamic Bitmaps', 'MyMiner'] # TODO: list of all schemes in the breakdown. Each column corresponds with one scheme
...
fig_ylim = (0, 6) # TODO: y-axis range. Align with your data
fig_yticks = list(np.arange(fig_ylim[0], fig_ylim[1], 1))


fig_bar_colors = ['#545b77', '#a6d0d0'] # TODO: choose enough colors for schemes
