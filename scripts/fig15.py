...
csv_path = './results/fig15.csv'
dataset_list = ['PP', 'YT', 'PT']
with open('./results/pimpam_threshold_time.txt', 'r') as f1:
    times = f1.readlines()
    times = [float(x.strip()) for x in times]

with open(csv_path, 'w') as f:
    i = 0
    f.write('Name,1,2,4,8,16,32,64\n')
    for graph in dataset_list:
        for pattern in ['CLIQUE3', 'TRI_TRI6']:
            part_times = times[i::6]
            normalized_times = [part_times[4]/x for x in part_times]
            f.write(graph + '-' + pattern + ',')
            f.write(','.join([str(x) for x in normalized_times]) + '\n')
            i += 1
...
fig_name = 'fig15' # TODO: figure name, used as pdf name


workload_list = csv_hd["Name"].to_list()
scheme_list = ['1', '2', '4', '8', '16', '32', '64'] # TODO: list of all schemes in the breakdown. Each column corresponds with one scheme
...
fig_ylim = (0, 1.4) # TODO: y-axis range. Align with your data
fig_yticks = list(np.arange(fig_ylim[0], fig_ylim[1], 0.25))


fig_bar_colors = ['#055da8', '#2e77b2', '#4687b8', '#81adc6', '#a6d0d0', '#e0e9dc', '#fefce4'] # TODO: choose enough colors for schemes
