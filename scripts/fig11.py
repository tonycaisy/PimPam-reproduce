...
upmem_path = './results/pimpam_time.txt'
graphpi_path = './results/graphpi_time.txt'
csv_path = './results/fig11.csv'
with open(csv_path, 'w') as f:
    with open(upmem_path, 'r') as f1:
        with open(graphpi_path, 'r') as f2:
            f.write("Name,CPU,MyMiner\n")
            for graph in ['WV', 'PP', 'AP', 'YT', 'PT', 'LJ']:
                for pattern in ['CLIQUE3', 'CLIQUE4', 'CYCLE4', 'HOUSE5', 'TRI_TRI6']:
                    f.write(graph + '-' + pattern + ',')
                    f.write(str(math.log10(float(f2.readline().strip()) * 1e6)) + ',')
                    f.write(str(math.log10(float(f1.readline().strip()) * 1e6)) + '\n')
...
fig_name = 'fig11' # TODO: figure name, used as pdf name

dataset_list = ['WV', 'PP', 'AP', 'YT', 'PT', 'LJ']

workload_list = csv_hd["Name"].to_list()
scheme_list = ['CPU', 'MyMiner'] # TODO: list of all schemes in the breakdown. Each column corresponds with one scheme
