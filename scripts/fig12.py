...
csv_path = './results/fig12.csv'
with open(csv_path, 'w') as f:
    with open('./results/pimpam_no_col.txt', 'r') as f1:
        with open('./results/pimpam_no_balance.txt', 'r') as f2:
            with open('./results/pimpam_time.txt', 'r') as f3:
                with open('./results/pimpam_ideal.txt', 'r') as f4:
                    f.write('Name,W/o Adaptive Collaboration,W/o Load-Aware Assignment,MyMiner,Ideal Load Balance\n')
                    for graph in dataset_list:
                        for pattern in ['CLIQUE3', 'CLIQUE4', 'CYCLE4', 'HOUSE5', 'TRI_TRI6']:
                            f.write(graph+'-'+pattern+',')
                            lines = [f1.readline(), f2.readline(), f3.readline(), f4.readline()]
                            lines = [float(line.strip()) for line in lines]
                            lines = [str(math.log10(line*1e6)) for line in lines]
                            f.write(','.join(lines) + '\n')
...
fig_name = 'fig12' # TODO: figure name, used as pdf name


workload_list = csv_hd["Name"].to_list()
scheme_list = ['W/o Adaptive Collaboration','W/o Load-Aware Assignment','MyMiner','Ideal Load Balance'] # TODO: list of all schemes in the breakdown. Each column corresponds with one scheme
...
fig_ylim = (0, 11) # TODO: y-axis range. Align with your data
fig_yticks = list(np.arange(fig_ylim[0], fig_ylim[1], 2))


fig_bar_colors = ['#ff6969','#ffd3b0', '#a6d0d0', '#1d5d9b', '#c9a7eb'] # TODO: choose enough colors for schemes
...
for i in range(0,6):
    next_ticks.append((fig_xticks[i*5] + fig_xticks[(i+1)*5-1])/2+0.02)
print(next_ticks)
...
line_ticks = []
for i in range(0,5):
    line_ticks.append((fig_xticks[(i+1)*5-1] + fig_xticks[(i+1)*5])/2+0.02)
...
fig_yticks_label = ["$10^{}$".format('{'+str(item)+'}') for item in fig_yticks] # Change for customized y tick label text
print(fig_yticks_label)
